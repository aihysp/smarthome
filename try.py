...
# Home Assistant WebSocket API settings
...
class CloseButton(QPushButton):
    pass  # Remove the mousePressEvent method override
...
class ReactStyleApp(QMainWindow):
    ...
    def turn_on_light(self):
        service_call_json = self.call_light_service("turn_on", "light.yeelight_ceiling5_0x0000000007cbc767")
        asyncio.get_event_loop().run_until_complete(send_command(service_call_json))

    def turn_off_light(self):
        service_call_json = self.call_light_service("turn_off", "light.yeelight_ceiling5_0x0000000007cbc767")
        asyncio.get_event_loop().run_until_complete(send_command(service_call_json))
